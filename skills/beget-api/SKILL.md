...
A server started with `--tool-sections` intentionally omits disabled categories. Treat `tools/list` as authoritative for the current connection; `beget://capabilities` and `beget_server_capabilities` report the same enabled surface. If a requested category is absent, explain that the server configuration must enable it. Never improvise a raw Beget API call.
