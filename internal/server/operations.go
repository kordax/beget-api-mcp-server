...
const (
	...
	getDNSDescription                  = "Read every active DNS record group for fqdn. Before a change, select one logical group and preserve all records inside that group. Do not pass this complete result back unchanged: omit other groups, empty arrays, and empty-value provider placeholders."
	changeDNSDescription               = "Replace exactly one logical Beget record group for fqdn per call: A/MX/TXT, NS, CNAME, or DNS/DNS_IP. First read with beget_get_dns_records, preserve every record in the selected group, apply the intended edit, and omit all other groups, empty arrays, and empty-value provider placeholders. Other logical groups remain unchanged. A dry run with dry_run=true and confirm=false is local and sends no Beget request. Verify afterward. Requires explicit confirm=true after user approval."
