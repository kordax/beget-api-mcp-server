...
Полный набор тестов, проверки покрытия, линтеров, поиска уязвимостей, статического анализа безопасности и поиска секретов запускается командой `task verify`. Перед первым запуском нужно установить зафиксированные версии инструментов через `task tools`. Обязательный порог покрытия составляет 90%, текущий набор тестов покрывает 97,6%, а badge публикуется из ветки `badges`. Те же категории проверок выполняются в GitHub Actions, а Dependabot следит за модулями Go и workflow.
