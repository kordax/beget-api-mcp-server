...
Команда `upgrade --check` только сравнивает версии, а конкретный выпуск можно указать как `upgrade v0.3.0`. Перед атомарной заменой исполняемого файла проверяется опубликованная SHA-256.
