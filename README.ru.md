...
MCP-сервер запускается и без настроенных учетных данных. Агент может вызвать `beget_auth_status`, определить состояние авторизации и получить безопасную инструкцию. Остальные инструменты Beget проверяют авторизацию только при вызове. Если системный keyring был временно недоступен во время запуска, сервер повторяет чтение, пока учетные данные отсутствуют. После успешной загрузки они остаются в памяти процесса до остановки сервера, поэтому временная последующая ошибка keyring не сбрасывает авторизацию. Установщик также добавляет Codex skill `beget-api`, который описывает этот порядок работы и запрещает передавать API-ключ через аргументы MCP.
